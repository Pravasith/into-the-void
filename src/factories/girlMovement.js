...
export const movements = {
    ...
    turnMultiplier: 0,
    init_ES6: function () {
        // a dummy function to access global 'this'
        ...
        this.init = presets => {
            ...
            const { models, anchor, document, camera, scene } = presets
            ...
            let terrainMesh = terrain.children.filter(
                item => item.name === "terrain"
            )[0]
        ...
        this.animateMovements = (keys, prevCurrKey) => {
            ...
            const boundary = terrain.children.filter(
                item => item.name === "boundary"
            )[0]

            if (boundary) {
                const boundaryIntersection =
                    girlRaycaster2.intersectObject(boundary)

                if (boundaryIntersection.length > 0) {
                    if (boundaryIntersection[0].distance >= 0.4)
                        positionStep = 0.1
                    else positionStep = 0
            ...
            if (
                Math.abs(this.fullRotCount.curr - this.fullRotCount.prev) >= 6
            ) {
                if (this.fullRotCount.curr - this.fullRotCount.prev > 0) {
                    // console.log("CLOCKWISE")
                    this.turnMultiplier--
                } else if (
                    this.fullRotCount.curr - this.fullRotCount.prev <
                    0
                ) {
                    this.turnMultiplier++
            ...
            if (keys[68]) {
                ...
                displace(
                    ...
                    new THREE.Vector3(
                        ...
                        axis,
                        (prevCurrKey[0] === 83 && prevCurrKey[1] === 68) ||
                            (prevCurrKey[0] === 68 && prevCurrKey[1] === 83)
            ...
            const setAnchorYPosWRTTerrain = (rayCasters, meshes) => {
                ...
                Array.max = intersectionPoints =>
                    Math.max.apply(Math, intersectionPoints)
            ...
            setAnchorYPosWRTTerrain(
                [girlToTerrainRaycaster1, girlToTerrainRaycaster2],
                terrainMeshes
            )
